...
A COG is a regular GeoTIFF file, aimed at being hosted on a HTTP file server, with an internal organization that enables more efficient workflows on the cloud.

Sentinel-2 is an Earth observation mission from the Copernicus Programme that acquires optical imagery at high spatial resolution (10 m to 60 m) over land and coastal waters.  It collects data in bands, which are different wavelengths of reflected light from earth surface and atmosphere.  Different bands can be used to study different aspects of the environment and can be used to calculate a range of [indexes](https://custom-scripts.sentinel-hub.com/custom-scripts/sentinel/sentinel-2/#remote-sensing-indices) that are known to represent key process.    
...
Only summer dates will be used because we want to ensure that leaves on the trees.  We end up averaging across all values in this time window.  The "T" separates the date and time component.  The "Z" is the time zone (Z by itself is UTC).
...
The search include the box and dates and a filter for clouds.  The post request sends the search query to the server where the catalog is hosted.  It returns an object that includes the list of images that match our criteria.  
...
In the items object, it provides the images (features) and bands (assets) that are available.
...
You likely had warning are associated with making sure the cube has regular sized box and time chunks.  This is a good feature because they help you get the right sized cube and will always widen the cube to include what you asked for.

The function `image_mask` creates an image mask based on values in a band.  Only pixels in the imagine that are not in this mask are used in our data cube.

By exploring <https://custom-scripts.sentinel-hub.com/custom-scripts/sentinel-2/scene-classification/>, we found that `scl` values of 3, 8, 9 correspond to cloud and cloud shadows and we want to filter out any pixels with those values.
...
The function `select_bands` selects the defined bands.  In our case, band selected also occurred at the call to  `stac_image_collection` above, so this select only removes the `scl` band.

The function `apply_pixel` applies the band-wise calculate to each pixel (e.g. calculates NDVI within each pixel in each image).  The function `reduce_time` finds the time dimension and applies the expression to collapse the time dimension down to a single value per x-y location.
...
Now, convert the ndvi object to a "stars" object so that it more easily can be visualized in a map.  A stars object is also a data cube with a slightly different format that plays well with mapping functions.  This step will take a while to run because it is the step where data is transferred from the remote server to your computer.  The speed that this step runs will depend your wifi speed or the network connection for the computer you are running it on.

```{r}
ndvi_stars <- st_as_stars(ndvi)
```

To not worry if you get the following warning `Warning: GDAL Message 1: The dataset has several variables that could be identified as vector fields, but not all share the same primary dimension. Consequently they will be ignored.`

Now plot the result. The long rectangle of Golden Gate Park is clearly visible in the North-West.

First, define the scales for the imagine using the `viridisLite::mako` palate. 

```{r}
mako <- tm_scale_continuous(values = viridisLite::mako(30))
```

Use the tmap package to visualize the image.  `tm_shape`  defines the shape (geographical shape) of the map.  `tm_raster` adds the raster data and uses the scale we defined above.  Note, because of running `reduce_time` above, our raster only has one value per location. `tmap` works like `ggplot2` where layers are added to a base plot.  
...
The function `st_read()` reads the polygon file from the the remote server. The file includes polygons of different administrated regions along with associated descriptive data.  It converts the data to a sf table.  Running `st_make_valid` is good practice because it ensures the data are correctly formatted.
...
We can use the `gdalcubes` package to extract data from the cube for each geometry (polygon).  In this case we are using the sf geometry from above.  We are calculating the mean in each polygon and letting it know to ignore the time component since we have already reduce in the time dimension.  

This step will take a while to run because it is the step where data is transferred from the remote server to your computer.  The speed that this step runs will depend your wifi speed or the network connection for the computer you are running it on.
...
Redlining was outlawed in 1968 following the passage of the civil rights bill.  Can we still see the legacy of this practice over 50 years later?

To answer the question we want to convert the sf object to a data frame (tibble) and use familiar dpylr functions to to calculate the statistics for each grade.
...
2) What would you change to adapt to new remote sensing index? (see <https://custom-scripts.sentinel-hub.com/custom-scripts/sentinel/sentinel-2/#remote-sensing-indices>)
